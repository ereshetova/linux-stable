...
stats_t cachefiles_lookup_histogram[HZ];
stats_t cachefiles_mkdir_histogram[HZ];
stats_t cachefiles_create_histogram[HZ];
...
static int cachefiles_histogram_show(struct seq_file *m, void *v)
{
	...
	default:
		...
		x = stats_read(&cachefiles_lookup_histogram[index]);
		y = stats_read(&cachefiles_mkdir_histogram[index]);
		z = stats_read(&cachefiles_create_histogram[index]);
		if (x == 0 && y == 0 && z == 0)
			return 0;
